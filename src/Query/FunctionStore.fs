...
module FunctionStore =
    ...
    let private fileName = "knowledge-sight.functions.json"
    ...
    /// Uses the same last-expression-return semantics as the eval IIFE wrapper.
            ...
            try
                ...
                let bodyJs =
                    if trimmed.Contains("return ") then
                        ...
                        // Mirror eval's IIFE logic: statements;expr → stmts + return expr
